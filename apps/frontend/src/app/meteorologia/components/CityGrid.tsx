...
import { FaNewspaper, FaMapMarkerAlt, FaCalendarAlt, FaExternalLinkAlt, FaSpinner } from 'react-icons/fa';
import { useWeatherNews, type WeatherNewsItem } from '@/hooks/useWeatherNews';
...
const FALLBACK_NEWS: Record<string, Array<{ title: string; source: string; type: 'alerta' | 'informe' | 'boletim'; url: string }>> = {
  'ilha-comprida': [
    { title: 'Avisos meteorológicos ativos para o litoral sul de SP', source: 'INMET', type: 'alerta', url: 'https://avisos.inmet.gov.br/' },
    { title: 'Previsão do tempo para Ilha Comprida — Próximos 7 dias', source: 'CPTEC/INPE', type: 'boletim', url: 'https://www.cptec.inpe.br/' },
  ],
  'iguape': [
    { title: 'Monitoramento de cheias no rio Iguape e estuário', source: 'Defesa Civil SP', type: 'informe', url: 'https://www.defesacivil.sp.gov.br/' },
    { title: 'Condições meteorológicas para Iguape — Boletim diário', source: 'INMET', type: 'boletim', url: 'https://portal.inmet.gov.br/' },
  ],
  'cananeia': [
    { title: 'Alerta de tempo severo para o litoral sul paulista', source: 'INMET', type: 'alerta', url: 'https://avisos.inmet.gov.br/' },
    { title: 'Previsão numérica para Cananéia — Modelo COSMO', source: 'CPTEC/INPE', type: 'boletim', url: 'https://previsaonumerica.cptec.inpe.br/' },
  ],
  'registro': [
    { title: 'Boletim climático do Vale do Ribeira', source: 'INMET', type: 'boletim', url: 'https://portal.inmet.gov.br/' },
    { title: 'Defesa Civil mantém monitoramento preventivo na região', source: 'Defesa Civil SP', type: 'informe', url: 'https://www.defesacivil.sp.gov.br/' },
  ],
};

function groupNewsByCity(news: WeatherNewsItem[]): Record<string, WeatherNewsItem[]> {
  const grouped: Record<string, WeatherNewsItem[]> = {};
  for (const item of news) {
    if (!grouped[item.cityId]) grouped[item.cityId] = [];
    grouped[item.cityId].push(item);
  }
  return grouped;
}
...
export function CityGrid({ onSelectLocation }: CityGridProps) {
  const { data, loading } = useWeatherNews();
  const apiNews = data?.news ?? [];
  const groupedApi = groupNewsByCity(apiNews);
  ...
  return (
    <section className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5" aria-label="Notícias meteorológicas por cidade">
      <div className="flex items-center gap-2 mb-4">
        ...
        {loading && <FaSpinner className="w-3 h-3 text-cyan-400 animate-spin ml-auto" aria-label="Carregando" />}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" role="list">
        {Object.entries(CITY_META).map(([cityId, meta]) => {
          const apiItems = groupedApi[cityId] ?? [];
          const fallbackItems = FALLBACK_NEWS[cityId] ?? [];
          ...
          return (
            <div
              key={cityId}
              role="listitem"
              title={`Notícias meteorológicas de ${meta.name} — ${meta.region}`}
            ...
            >
                  ...
                  ? apiItems.slice(0, 2).map((item) => {
                      ...
                      return (
                        <a
                          ...
                          aria-label={`${item.title} — ${item.source}, ${formatDate(item.publishedAt)}`}
                        >
                          <div className="flex items-start gap-2">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="flex items-center gap-1.5 mt-1">
                                ...
                                <span className="text-[10px] text-slate-500">{formatDate(item.publishedAt)}</span>
              ...
              <button
                ...
                title={`Clique para ver a previsão completa de ${meta.name}`}
                ...
                aria-label={`Selecionar ${meta.name} para ver previsão`}
              >
